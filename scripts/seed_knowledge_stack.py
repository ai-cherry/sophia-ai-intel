...
Embeddings via standardized Portkey routing - OpenRouter removed.
...
class EmbeddingClient:
    """Standardized embedding client using Portkey virtual keys"""
    ...
    async def create_embedding(self, text: str, attempt: int = 0) -> List[float]:
        """Create embedding using standardized Portkey routing"""

        if attempt >= MAX_EMBEDDING_RETRIES:
            raise Exception("Max embedding retries exceeded")

        if not self.portkey_available:
            logger.warning("Portkey API key not configured - using mock embedding")
            return [0.1] * 1536  # Mock 1536-dimensional embedding

        # Use Portkey with standardized virtual key routing
        try:
            ...
            if attempt < MAX_EMBEDDING_RETRIES - 1:
                ...
                logger.warning("Using mock embedding - all API attempts failed")
                return [0.1] * 1536  # Mock 1536-dimensional embedding

    async def _portkey_embedding(self, text: str) -> List[float]:
        """Create embedding via Portkey standardized routing"""
        payload = {
...
class KnowledgeStack:
    ...
    async def store_document_chunks(self, chunks: List[Dict[str, Any]]) -> tuple[int, int]:
        ...
        async with self.embedding_client as emb_client:
            for chunk in chunks:
                try:
                    # Create embedding using standardized routing
